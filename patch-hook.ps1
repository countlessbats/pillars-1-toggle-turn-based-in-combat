# Injects LoomTurnBasedToggle.Bootstrap.Tick() at the top of GameState.Update() in Assembly-CSharp.dll.
# Idempotent: skips if the LoomTurnBasedToggle assembly reference is already present.
...
$sidecar = Join-Path $managed 'LoomTurnBasedToggle.dll'
...
try {
    if ($module.AssemblyReferences | Where-Object { $_.Name -eq 'LoomTurnBasedToggle' }) {
        Write-Host "Already patched (LoomTurnBasedToggle reference present). Nothing to do." -ForegroundColor Yellow
        return
    }
    ...
    $bootstrap = $sc.MainModule.Types | Where-Object { $_.FullName -eq 'LoomTurnBasedToggle.Bootstrap' } | Select-Object -First 1
    if (-not $bootstrap) { throw "LoomTurnBasedToggle.Bootstrap not found in sidecar." }
    ...
    $anr = New-Object Mono.Cecil.AssemblyNameReference('LoomTurnBasedToggle', $sc.Name.Version)
    $module.AssemblyReferences.Add($anr)

    $backup = "$asmPath.pre-tbtoggle-backup"
    if (-not (Test-Path $backup)) { Copy-Item -LiteralPath $asmPath -Destination $backup -Force }

    $tmp = "$asmPath.tbtoggle-patched"
